    ...
    <div className="flex w-11/12 mt-2 justify-between items-start gap-8 flex-col md:flex-row">
        ...
        {(provided, snapshot) => (
          <div
            className={`flex flex-col w-full p-3 rounded-md md:w-1/2 ${
              snapshot.isDraggingOver ? 'bg-green-700' : 'bg-green-700'
            } md:w-1/2`}
            ref={provided.innerRef}
            {...provided.droppableProps}
          >
            <span className="text-white text-2xl p-2 font-custom">Avtive Tasks</span>
      ...
      <Droppable droppableId="complete">
        {(provided, snapshot) => (
          <div
          className={`flex flex-col w-full p-3 rounded-md md:w-1/2 ${
            snapshot.isDraggingOver ? 'bg-red-700' : 'bg-red-700'
          } md:w-1/2`}
            ref={provided.innerRef}
            {...provided.droppableProps}
          >
